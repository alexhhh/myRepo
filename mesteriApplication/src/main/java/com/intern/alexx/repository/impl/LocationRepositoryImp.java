...
public class LocationRepositoryImp implements LocationRepository{
	...
	public void insert(Location location) {
		...
		template.update(sql, new Object[] { location.getId(),location.getMesterId(), location.getLocation(), location.getLatitude(), location.getLongitude() });		
	}

	@Override
	public void update(Location location) {
	 String sql= "UPDATE location SET LOCATION=?, LATITUDE=?, LONGITUDE=? WHERE mester_id=?";
	 template.update(sql, new Object[] {  location.getLocation(), location.getLatitude(), location.getLongitude(),location.getMesterId()});		
	...
	public Location getById(String id) {
		String sql="Selet * from location where mester_id=?";
	...
	public List<Location> getAllLocations() {
		...
		for (Map<String, Object> row : rows) {
			Location location = new Location();
			locationRowMapper(location, row);
	...
	public List<Location> getLocationsByIds(List<String> ids) {
		...
		String sql = " SELECT * FROM location WHERE mester_id IN (:ids)"; 
		  locations = namendTemplate.query(sql , params, rm ) ; 
		return locations;
	}
	...
	private Location locationRowMapper(Location location , Map<String, Object> row){		 
