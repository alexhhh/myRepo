...
public class ClientEndpoint {
	...
	@GET
	@Path("/query")
	@ApiOperation(value = "Return OK", notes = "Return OK if client is in the db", response = Client.class)
	@ApiResponses(value = { 
			@ApiResponse(code = 200, message = "Client found.", response = Client.class),
			@ApiResponse(code = 401, message = "Not authorized"),
			@ApiResponse(code = 404, message = "User not found"),
			@ApiResponse(code = 500, message = "Internal server error") })
	public Response checkClient(@QueryParam("id") String id) {
		Client dbClient = clientService.getClientById(id);
	...
	@DELETE
	@ApiOperation(value = "Delete a client by id", notes = "Delete client by id", response = Client.class)
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Client was successfuly deleted.", response = Client.class),
			@ApiResponse(code = 500, message = "Internal server error.") })
