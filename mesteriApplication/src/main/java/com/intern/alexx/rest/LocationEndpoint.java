...
public class LocationEndpoint {
	...
	@GET
	@Path("/all")
	@ApiOperation(value = "Get locations", notes = "Get locations.", response = Location.class)
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "All locations was successfully retrived.", response = Location.class),
	...
	@POST
	@Path("/new")
	@ApiOperation(value = "Add locations", notes = "Add locations.", response = Location.class)
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "The locations was successfully added.", response = Location.class),
			@ApiResponse(code = 404, message = "Locations  not found."),
			@ApiResponse(code = 500, message = "Internal server error") })
	public Response insertLocation(Location location ) throws SQLException {		 
	...
	public Response getLocationsByIds(List <String> ids ) throws SQLException {		
		...
		List<Location> locations = locationService.getLocationsByIds(ids);
		return Response.ok(Status.OK).entity(locations).build();
	}
	
	@PUT
	@Path("/mester")
	@ApiOperation(value = "Update location ", notes = "Update location.", response = Location.class)
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Location was successfully updated.", response = Location.class),
			@ApiResponse(code = 404, message = "Location was not found."),
			@ApiResponse(code = 500, message = "Internal server error.") })
	public Response editLocation(Location location) {
	...
	@GET
	@Path("/mester/query")
	@ApiOperation(value = "Get the  mester location", notes = "Get location.", response = Location.class)
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Mester location was successfully retrived.", response = Location.class),
	...
	public Response getMesterSpecialities(@QueryParam("mesterId") String mesterId) throws SQLException {
		Location location = locationService.getById(mesterId);
		return Response.ok(Status.OK).entity(location).build();
	}
